...
if [ ! -d "$DIR" ]; then
  mkdir -p "$DIR" && echo 'jira.shared.home=/var/atlassian/application-data/cluster' >> /var/atlassian/application-data/jira/jira-main-home/cluster.properties && /bin/bash /var/atlassian/application-data/jira/mkdir-home4.sh && MY_POD_NAME="jira-main-home"
else 
